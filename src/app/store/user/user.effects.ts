...
export class UserEffects {
    ...
    constructor(private actions$: Actions, private router: Router, private service: UsersService) { }
    loadUsers$ = createEffect(() => this.actions$.pipe(
        ofType(userActions.loadUsers),
        mergeMap(() => this.service.getAllUsers().pipe(
    ...
    signInUser$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(({ body }) =>
            this.service.signInUser(body).pipe(
                ...
                catchError((error) =>
                    of(userActions.signInUserFailure(error))
                )
            )
        )
    ))
    ...
    getCurrentUser$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(() => this.service.getUser().pipe(
            map(user => userActions.getCurrentUserSuccess({ currentUser: user, isLoggedIn: localStorage.getItem("isLoggedIn") })),
            catchError(error => of(userActions.getCurrentUserFailure({ currentUser: null, error: error })))
        ))
    ))
    ...
    updateUserRole$ = createEffect(() => this.actions$.pipe(
        ...
        map(({ updateUserRole }) => userActions.updateUserRoleSuccess({ updateUserRoles: updateUserRole })),
    ...
    registerUser$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(({ user }) =>
            this.service.registerUser(user).pipe(
                ...
                map(createdUser => userActions.signInUserSuccess({
